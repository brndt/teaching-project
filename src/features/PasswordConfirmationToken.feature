Feature: Password confirmation token

  Scenario: Password reset confirmation when all inputs are valid
    ...
    """
    {
      "newPassword": "qwertY67",
      "confirmationToken": "6e4965317dd91ed1edee4af180b03775c84177c4ac82b63fe4e0c8e418e5832458036a89aa9f7c620ca51dc9f8606abc499ce018075662e4b4f4ad44ed6ed708"
    }
    """
  ...
  Scenario: Password reset confirmation when expiration date of confirmation token is invalid
    Given there are users with the following details:
      | id                                   | firstName | lastName    | email             | password | roles | confirmationToken                                                                                                                | expirationDate |
  ...
  Scenario: Password reset confirmation when confirmation tokens are different
    Given there are users with the following details:
      | id                                   | firstName | lastName    | email             | password | roles | confirmationToken                                                                                                                | expirationDate |
